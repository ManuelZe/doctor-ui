...
interface RealisationActuelProps {
...
export function RealisationActuel({ list_patient_name, montant_prescription, montant_realisation, montant_total, nombre_patient, message, ...props }: RealisationActuelProps) {
  return (
    <div className="grid grid-cols-2 gap-2 bg-blue-100/50 aspect-video rounded-xl text-center pb-2">
      <div className="col-span-2 col-start-1 pl-4 pt-4">
        <h1 className="text-l tracking-tight text-balance text-left font-semibold ">Montant Réalisation</h1>
      ...
      <div className="justify-center-safe justify-items-center">
        <h1 className="scroll-m-20 text-xl font-bold tracking-tight text-balance">
              {montant_realisation ? Number(montant_realisation).toLocaleString('fr-FR') + ' FCFA' : '0 FCFA'}
        </h1>
      </div>
